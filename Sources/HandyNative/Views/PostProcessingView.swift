...
struct PostProcessingView: View {
    ...
    private var apiGroup: some View {
        HandySettingsGroup("API (OpenAI Compatible)") {
            HandySettingRow("Provider", description: "Select an OpenAI-compatible provider.") {
            ...
            if selectedProvider.allowBaseURLEdit {
                HandyDivider()
                HandySettingRow("Base URL", description: "API base URL for the selected provider.") {
            ...
            if isAppleProvider {
                ...
                HandyDivider()
                HandySettingRow("API Key", description: "API key for the selected provider.") {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Button {
                            ...
                            if appModel.isFetchingPostProcessModels {
                                SpinningHandyHugeIcon(kind: .refresh, color: HandyDesign.text, size: 16)
                            } else {
                                HandyHugeIcon(kind: .refresh, color: HandyDesign.text, size: 16)
                            }
                        }
                        .buttonStyle(HandyButtonStyle(variant: .secondary))
    ...
    private var promptEmptyState: some View {
        Text(appModel.settings.postProcessPrompts.isEmpty ? "Click 'Create New Prompt' above to create your first post-processing prompt." : "Select a prompt above to view and edit its details.")
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(12)
            .background(HandyDesign.midGray.opacity(0.05))
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Tip: Use ${output} to insert the transcribed text in your prompt.")
                    ...
                    .foregroundStyle(HandyDesign.midGray.opacity(0.7))
            }
