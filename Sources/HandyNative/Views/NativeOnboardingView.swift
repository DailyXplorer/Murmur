    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Handy needs a couple of permissions to work properly.")
                    .font(HandyDesign.font(size: 14))
                    .foregroundStyle(HandyDesign.text.opacity(0.7))
            ...
            if returningUser {
                Text("Your transcription model is ready. Grant permissions to continue.")
                    ...
                    .foregroundStyle(HandyDesign.text.opacity(0.55))
            }
        }
        .padding(24)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
...
private struct PermissionCard: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 16) {
            HandyHugeIcon(kind: icon, color: handyTheme.logoPrimary(for: colorScheme), size: 24)
                .frame(width: 48, height: 48)
                .background(handyTheme.logoPrimary(for: colorScheme).opacity(0.2))
                .clipShape(Circle())

            VStack(alignment: .leading, spacing: 6) {
                Text(title)
                    .font(HandyDesign.font(size: 15, weight: .medium))
                    .foregroundStyle(HandyDesign.text)
                Text(description)
                    .font(HandyDesign.font(size: 13))
                ...
                if isGranted {
                    ...
                    PermissionStatusLabel(
                        text: "Waiting...",
                        icon: .loading,
                        color: HandyDesign.text.opacity(0.55),
                        spinning: true
                    )
                } else {
...
private struct PermissionStatusLabel: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            if spinning {
                SpinningHandyHugeIcon(kind: icon, color: color, size: 16)
            } else {
                HandyHugeIcon(kind: icon, color: color, size: 16)
            }
...
private struct OnboardingModelCard: View {
    ...
    private var background: Color {
        if status == "Active" || isRecommended {
            return handyTheme.logoPrimary(for: colorScheme).opacity(status == "Active" ? 0.1 : 0.05)
        }
        return .clear
    }
    ...
    private var border: Color {
        if status == "Active" {
            return handyTheme.logoPrimary(for: colorScheme).opacity(0.5)
        }
        if isRecommended {
            return handyTheme.logoPrimary(for: colorScheme).opacity(0.25)
        }
        return HandyDesign.midGray.opacity(0.2)
    }
}
...
private struct ModelCapabilityLabel: View {
    ...
    init(_ text: String, icon: HandyHugeIconKind) {
        self.text = text
        self.icon = icon
    }
    ...
    var body: some View {
        HStack(spacing: 4) {
            HandyHugeIcon(kind: icon, color: HandyDesign.text.opacity(0.5), size: 14)
            Text(text)
        }
    }
}
...
private struct Badge: View {
    ...
    var icon: HandyHugeIconKind?

    init(_ text: String, emphasized: Bool = false, icon: HandyHugeIconKind? = nil) {
        self.text = text
    ...
    var body: some View {
        HStack(spacing: 4) {
            if let icon {
                HandyHugeIcon(kind: icon, color: HandyDesign.text, size: 12)
            }
            Text(text)
        }
...
private struct OnboardingLogo: View {
    var body: some View {
        HandyLogoView(width: 200, height: 92)
    }
}
