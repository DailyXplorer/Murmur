...
final class NativeLaunchArgumentsTests: XCTestCase {
    func testPasteSmokeOutputEncodesJSONContractWithoutClipboardContents() throws {
        let output = NativePasteSmokeOutput(
            ...
            activatedApplicationBundleIdentifier: "computer.murmur.Murmur",
            activatedApplicationLocalizedName: "Murmur",
            accessibilityWindowFocusSucceeded: true,
            frontmostApplicationProcessIdentifier: 12345,
            frontmostApplicationBundleIdentifier: "computer.murmur.Murmur",
            frontmostApplicationLocalizedName: "Murmur"
        )
        ...
        XCTAssertEqual(object["activatedApplicationBundleIdentifier"] as? String, "computer.murmur.Murmur")
    ...
    func testExternalPasteRoundTripSmokeOutputEncodesJSONContract() throws {
        let output = NativeExternalPasteRoundTripSmokeOutput(
            ...
            paste: NativePasteSmokeOutput(
                ...
                activationSucceeded: true,
                activatedApplicationBundleIdentifier: "com.pais.murmur",
                activatedApplicationLocalizedName: "Murmur",
                accessibilityWindowFocusSucceeded: true,
                frontmostApplicationProcessIdentifier: 456,
                frontmostApplicationBundleIdentifier: "com.pais.murmur",
                frontmostApplicationLocalizedName: "Murmur"
            ),
    ...
    func testRemoteControlListenerSmokeOutputEncodesRoundTripContract() throws {
        let output = NativeRemoteControlListenerSmokeOutput(
            ...
            senderLaunchPath: "/tmp/murmur-native-dist/Murmur.app",
            bundleIdentifier: "com.pais.murmur",
        ...
        XCTAssertEqual(object["senderLaunchPath"] as? String, "/tmp/murmur-native-dist/Murmur.app")
        XCTAssertEqual(object["bundleIdentifier"] as? String, "com.pais.murmur")
    ...
    func testOpenSectionArgumentAcceptsProductLabelsAndSeparators() {
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--open-section=post-process"]).initialSection,
            .postProcessing
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--open-section", "Post Process"]).initialSection,
        ...
        XCTAssertNil(
            NativeLaunchArguments.parse(["Murmur", "--open-section", "missing"]).initialSection
        )
    }
    ...
    func testRemoteCommandParsesPostProcessAndCancel() {
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--toggle-post-process"]).remoteCommand,
            .togglePostProcess
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--cancel"]).remoteCommand,
            .cancel
        )
    }
    ...
    func testParseModelCacheSmokeArguments() {
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-model-cache-status", "tiny"]).smokeModelCacheRequest,
            NativeModelCacheSmokeRequest(modelID: "tiny", operation: .status)
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-delete-model-cache=base"]).smokeModelCacheRequest,
            NativeModelCacheSmokeRequest(modelID: "base", operation: .delete)
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-download-model-cache", "tiny"]).smokeModelCacheRequest,
    ...
    func testParseOverlaySmokeArguments() {
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-overlay-state", "recording"]).smokeOverlayState,
            .recording
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-overlay-state=processing"]).smokeOverlayState,
            .processing
        )
        XCTAssertNil(
            NativeLaunchArguments.parse(["Murmur", "--smoke-overlay-state", "missing"]).smokeOverlayState
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse([
                "Murmur",
        ...
        XCTAssertNil(
            NativeLaunchArguments.parse(["Murmur", "--smoke-output-json", "/tmp/overlay.json"]).smokeOverlayOutputPath
        )
        XCTAssertNil(
            NativeLaunchArguments.parse(["Murmur", "--smoke-output-image", "/tmp/overlay.png"]).smokeOverlayImageOutputPath
        )
    }

    func testParseOnboardingSmokeArguments() {
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-onboarding-step", "permissions"]).smokeOnboardingStep,
            .permissions(returningUser: false)
        )
        XCTAssertEqual(
            NativeLaunchArguments.parse(["Murmur", "--smoke-onboarding-step=model"]).smokeOnboardingStep,
            .model
        )
        XCTAssertNil(
            NativeLaunchArguments.parse(["Murmur", "--smoke-onboarding-step", "missing"]).smokeOnboardingStep
        )
    }
